...
const regexToDb = /function toDbFieldName\(path, fieldName\) \{[\s\S]*?return fieldName\.replace\(\/\[A-Z\]\/g, letter => `_\$\{letter\.toLowerCase\(\)\}`\);\n\}/;
...
code = code.replace(regexToDb, newToDb);

const regexFromDb = /function fromDbFieldName\(path, fieldName\) \{[\s\S]*?return fieldName\.replace\(\/_\\(\[a-z\]\\)\/g, \(_, letter\) => letter\.toUpperCase\(\)\);\n\}/;
const newFromDb = `function fromDbFieldName(path, fieldName) {
    ...
    if (fieldName === 'image_url') return 'imageUrl';
    if (fieldName === 'sort_order') return 'sortOrder';
...
code = code.replace(regexFromDb, newFromDb);
